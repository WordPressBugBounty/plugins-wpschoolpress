				...
				<table id="teacher_table" class="wpsp-table">
					...
					<tbody>
						...
						if (!empty($classID)) {
							...
							$classquery = 'AND c.cid = %d';

							$sub_han = $wpdb->get_results(
								$wpdb->prepare(
									"SELECT sub_name, sub_teach_id, c.c_name 
										FROM $subjects_table s, $class_table c 
										WHERE sub_teach_id > 0 
										AND c.cid = s.class_id 
										$classquery 
										ORDER BY c.cid",
									$classID
								)
							...
							$incharges = $wpdb->get_results(
								$wpdb->prepare(
									"SELECT c.c_name, c.teacher_id 
									FROM $class_table c 
									LEFT JOIN $teacher_table t ON t.wp_usr_id = c.teacher_id 
									WHERE c.teacher_id > 0 
									$classquery",
									$classID
								)
							);
							...
							if (!empty($teacher) && !empty($classID)) {
								$teacherPlaceholders = implode(', ', array_fill(0, count($teacher), '%d'));
								$teacherQuery = ' WHERE wp_usr_id IN (' . $teacherPlaceholders . ')';

								$teachers = $wpdb->get_results(
									$wpdb->prepare(
										"SELECT * FROM $teacher_table $teacherQuery ORDER BY tid DESC",
										$teacher
									)
								);
							} else {
						...
						?>
								<tr>
									...
									<td><?php if (isset($cincharge[$tinfo->wp_usr_id])) {
											echo esc_html(implode(", ", $cincharge[$tinfo->wp_usr_id]));
										} else {
											echo '-';
										} ?></td>
									<td><?php if (isset($sub_handling[$tinfo->wp_usr_id])) {
											echo esc_html(implode(", ", $sub_handling[$tinfo->wp_usr_id]));
										} else {
											echo '-';
										} ?></td>
