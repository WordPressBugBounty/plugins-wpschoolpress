...
if( is_user_logged_in() ) {
	...
	if($current_user_role=='administrator' || $current_user_role=='teacher'){
		...
		<div class="wpsp-card">
			...
			<div class="wpsp-card-body text-black">
				<div class="wpsp-row">
					<?php $item =  esc_html(apply_filters( 'wpsp_student_attendance_title_item',"Attendance"),"wpschoolpress"); ?>
							...
							<div class="wpsp-form-group">
								...
								<select name="classid" id="AttendanceClass" class="wpsp-form-control">
									...
									//	$clt=$wpdb->get_results("select `cid`,`c_name` from `$ctname` where teacher_id = '$current_user_id' ");
										$clt = $wpdb->get_results($wpdb->prepare("SELECT `cid`,`c_name` FROM $ctname WHERE teacher_id = %d",$current_user_id));
										foreach($clt as $cnm){?>
					...
					//	$attendance_info	=	$wpdb->get_results("select class_id, absents from $attendance_table where date='$date_today'");
						$attendance_info = $wpdb->get_results($wpdb->prepare("SELECT class_id, absents FROM $attendance_table WHERE date = %s",$date_today));
	...
	//	$students=$wpdb->get_results("select wp_usr_id, class_id, s_fname, s_mname, s_lname, sid from $student_table where parent_wp_usr_id='".esc_sql($parent_id)."'");
		...
		$students = $wpdb->get_results($wpdb->prepare("SELECT wp_usr_id, class_id, s_fname, s_mname, s_lname, sid FROM $student_table WHERE parent_wp_usr_id = %d",$parent_id));
			...
			<div class="wpsp-card-body">
				<div class="tabSec wpsp-nav-tabs-custom" id="verticalTab">
							...
							<?php $i=0; foreach($child as $ch) {
								if(base64_decode(sanitize_text_field($_GET['sid'])) == $ch['sid']){ ?>
									<li class="wpsp-tabing <?php echo ($i==0)?'wpsp-resp-tab-active':''?>">
										<?php echo esc_html($ch['s_fname'].' '.$ch['s_mname'].' '. $ch['s_lname']);?>
									</li>
								<?php }$i++;
							 } ?>
						</ul>
					...
					<?php
						...
						$class_table = $wpdb->prefix . "wpsp_class";
						$st_wp_id = $wpdb->get_row($wpdb->prepare("select wp_usr_id from $st_table where sid = '%d'",$st_id));
						$wp_usr_id = $st_wp_id->wp_usr_id;
						$ser = '%' . $wp_usr_id . '%';
					//	$stinfo = $wpdb->get_row("select st.class_id, st.s_rollno, st.wp_usr_id, CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname )
					//	 AS full_name, c.c_name, c.c_sdate, c.c_edate from $st_table st LEFT JOIN $class_table c ON c.cid='".esc_sql($class_id)."' where st.sid='".esc_sql($st_id)."'");

						$stinfo = $wpdb->get_row($wpdb->prepare("SELECT st.class_id, st.s_rollno, st.wp_usr_id, CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name, c.c_name, c.c_sdate, c.c_edate 
         					FROM $st_table st LEFT JOIN $class_table c ON c.cid = %d WHERE st.sid = %d",$class_id,$st_id));	 
					...
					//	$att_info = $wpdb->get_row("select count(*) as count from $att_table WHERE absents LIKE ".esc_sql($ser)." AND class_id='$class_id'");
						$att_info = $wpdb->get_row($wpdb->prepare("SELECT count(*) as count FROM $att_table WHERE absents LIKE %s and class_id=%d",$ser,$class_id));
						...
						//	$clasname = $wpdb->get_var("SELECT c_name FROM $class_table where cid='".esc_sql($class_id)."'");
							$clasname = $wpdb->get_var($wpdb->prepare("SELECT c_name FROM $class_table WHERE cid = %d", $class_id));
		...
		<div class="wpsp-card">
			<div class="wpsp-card-body">
				<section class="content">
					...
					$class_table = $wpdb->prefix . "wpsp_class";
					$ser = '%' . $st_id . '%';
					//$stinfo = $wpdb->get_row("select st.class_id, st.s_rollno , CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name, c.c_name, c.c_sdate, c.c_edate from $st_table
					// st LEFT JOIN $class_table c ON c.cid='".esc_sql($class_id)."' where st.wp_usr_id='".esc_sql($st_id)."'");

					$stinfo = $wpdb->get_row($wpdb->prepare("SELECT st.class_id, st.s_rollno, CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name, c.c_name, c.c_sdate, c.c_edate 
         					FROM $st_table st LEFT JOIN $class_table c ON c.cid = %d WHERE st.wp_usr_id = %d",$class_id,$st_id));	 
