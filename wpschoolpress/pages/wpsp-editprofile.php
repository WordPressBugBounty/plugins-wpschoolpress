...
if( is_user_logged_in() ) {
    ...
    $tinfo = $wpdb->get_row($wpdb->prepare("SELECT teacher.*,user.user_email FROM $teacher_table teacher LEFT JOIN $users_table user ON user.ID=teacher.wp_usr_id WHERE teacher.wp_usr_id = %d",$sid));
    if( !empty( $stinfo ) ) {
    ...
    if($current_user_role == 'parent'){
       ...
       $stinfo = $wpdb->get_row($wpdb->prepare("SELECT * FROM $student_table WHERE parent_wp_usr_id = %d",$sid));
       $loc_avatar=get_user_meta($sid,'simple_local_avatar',true);
       $img_url= $loc_avatar ? $loc_avatar['full'] : WPSP_PLUGIN_URL.'img/default_avtar.jpg';
    }
    ?>
                        ...
                        <div class="wpsp-row">
                            ...
                            <div class="wpsp-col-lg-3 wpsp-col-md-3 wpsp-col-sm-4 wpsp-col-xs-12">
                                <div class="wpsp-form-group">
                                    ...
                                    <input type="text" class="wpsp-form-control" id="s_p_phone" name="s_p_phone" value="<?php echo esc_attr($stinfo->s_phone);?>" placeholder="Phone Number" onkeypress='return event.keyCode == 8 || event.keyCode == 46
