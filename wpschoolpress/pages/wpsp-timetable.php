...
if (is_user_logged_in()) {
	...
	if( $current_usr_rle == 'administrator' || $current_usr_rle == 'teacher' ) {
								...
								<div class="wpsp-col-lg-3 wpsp-col-md-4 wpsp-col-sm-4 wpsp-col-xs-3s">
									<div class="wpsp-form-group">
										<?php if( isset( $response['status'] ) && $response['status']==2 ) { ?>
											<a href="?page=sch-timetable&timetable=<?php echo esc_attr($sel_classid); ?>" title="Edit" data-id="<?php echo esc_attr($sel_classid); ?>" class="gap-all wp-edit-timetable wpsp-timetable-btn">
												<i class="icon wpsp-edit wpsp-edit-icon icn-gap"></i>
											</a>
											<a href="javascript:void(0);" title="Delete" data-id="<?php echo esc_attr($sel_classid); ?>" title="Delete" class="wp-delete-timetable wpsp-timetable-btn">
		...
		wpsp_body_end();
		wpsp_footer();
	} else if ($current_usr_rle == 'student') {
		wpsp_topbar();
		wpsp_sidebar();
		wpsp_body_start();
		include_once(  WPSP_PLUGIN_PATH.'/includes/wpsp-viewTimetable.php');
		...
		include_once( WPSP_PLUGIN_PATH.'/includes/wpsp-viewTimetable.php');
		$wpsp_student_table = $wpdb->prefix . "wpsp_student";
		$child_info = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpsp_student_table WHERE s.parent_wp_usr_id = %d",$current_user->ID));
		?>
