                    ...
                    <div class="wpsp-card-body">
                        <table id="class_table" class="wpsp-table wpsp-table-bordered wpsp-table-striped" cellspacing="0" width="100%" style="width:100%">
                            ...
                            <tbody>
                                ...
                                $wpsp_stud_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM $stable s
                                INNER JOIN $ctlass c
                                where s.wp_usr_id WHERE s.wp_usr_id = %d",$user_id));
                                ...
                                $proid = 0;
                                $wpsp_fees_data = $wpdb->get_results("SELECT s.wp_usr_id,f.student_id,f.order_id FROM $stable AS s INNER JOIN wp_wpsp_fees AS f ON f.student_id =  s.wp_usr_id");
                                ...
                                foreach($wpsp_clas_data as $clsloop){
                                    ...
                                    $wpsp_teacher_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM $cteacher WHERE wp_usr_id = %d",$clsloop->teacher_id));
                                    ...
                                    $wpsp_order_data = $wpdb->get_results($wpdb->prepare("SELECT c.cid,f.class_id,f.fees_amount,f.created_date FROM $ctlass as c INNER JOIN wp_wpsp_fees AS f ON f.class_id = c.cid where f.class_id = %d",$clsloop->teacher_id));
                ...
                <div class="wpsp-card">
                    ...
                    <div class="wpsp-card-body">
                        <table id="class_table" class="wpsp-table wpsp-table-bordered wpsp-table-striped" cellspacing="0" width="100%" style="width:100%">
                            ...
                            <tbody>
                                ...
                                if($current_user_role == 'parent'){
                                  //  $user_id = $wpdb->get_var("SELECT wp_usr_id FROM $stable where parent_wp_usr_id = '".esc_sql($current_user->ID)."'");
                                    $user_id = $wpdb->get_var($wpdb->prepare("SELECT wp_usr_id FROM $stable WHERE parent_wp_usr_id = %d",$current_user->ID));
                                }else {
                                    $user_id = esc_sql($current_user->ID);
                                }
                                if($current_user_role == 'parent'){
                                   // $wpsp_stud_data =$wpdb->get_results("SELECT * FROM  $stable s INNER JOIN $ctlass c where s.parent_wp_usr_id = '".$current_user->ID."' and c.c_fee_type = 'paid'");
                                    $wpsp_stud_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM $stable s INNER JOIN $ctlass c where s.parent_wp_usr_id = %d and c.c_fee_type = %s",$current_user->ID,'paid'));    
                                }else {
                                    $wpsp_stud_data =$wpdb->get_results("SELECT * FROM  $stable s INNER JOIN $ctlass c where s.wp_usr_id = '".$user_id."' and c.c_fee_type = 'paid'");
                                    $wpsp_stud_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM $stable s INNER JOIN $ctlass c where s.wp_usr_id = %d and c.c_fee_type = %s",$user_id,'paid'));        
                                }
                                $classid_da = maybe_unserialize($wpsp_stud_data[0]->class_id);
                                $cid = [];
                            ...
                            //    $wpsp_teacher_data = $wpdb->get_results("SELECT * FROM  $cteacher where wp_usr_id = '".$clsloop->teacher_id."'");
                                $wpsp_teacher_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM $cteacher WHERE wp_usr_id = %d",$clsloop->teacher_id));
                             ...
                             //   $wpsp_fees_data = $wpdb->get_results("SELECT s.wp_usr_id,f.student_id,f.class_id,f.order_id FROM $stable AS s INNER JOIN wp_wpsp_fees AS f ON f.student_id =  s.wp_usr_id where s.parent_wp_usr_id = '".$current_user->ID."'");
                                $wpsp_fees_data = $wpdb->get_results($wpdb->prepare("SELECT s.wp_usr_id,f.student_id,f.class_id,f.order_id FROM $stable AS s INNER JOIN wp_wpsp_fees AS f ON f.student_id = s.wp_usr_id WHERE s.parent_wp_usr_id = %d",$current_user->ID));
                                foreach($wpsp_fees_data as $res){
                                    $siid[] = $res->wp_usr_id;
                                    $ciid = $res->class_id;
                                }
                              //  $wpsp_stud =$wpdb->get_results("SELECT * FROM  $stable s where s.parent_wp_usr_id = '".esc_sql($current_user->ID)."' ORDER BY s.wp_usr_id DESC");
                                $wpsp_stud = $wpdb->get_results($wpdb->prepare("SELECT * FROM $stable WHERE s.parent_wp_usr_id = %d ORDER BY s.wp_usr_id DESC",$current_user->ID));
                                $wpsp_tech =$wpdb->get_results("SELECT s.class_id,c.cid,c.teacher_id FROM $ftlass AS s INNER JOIN $ctlass AS c ON c.cid=s.class_id");
                                ...
                                foreach($wpsp_stud as $value){
                                    if(in_array($value->wp_usr_id, $siid)){
                                      //  $wpsp_cls_data =$wpdb->get_results("SELECT s.order_id,s.fees_amount,s.created_date,s.student_id,s.class_id,c.cid,c.c_name,c.teacher_id  FROM  wp_wpsp_fees AS s INNER JOIN wp_wpsp_class AS c ON c.cid=s.class_id where student_id = '".esc_sql($value->wp_usr_id)."' ORDER BY s.student_id DESC");
                                        $wpsp_cls_data = $wpdb->get_results($wpdb->prepare("SELECT s.order_id,s.fees_amount,s.created_date,s.student_id,s.class_id,c.cid,c.c_name,c.teacher_id FROM $ftlass AS s INNER JOIN $ctlass AS c ON c.cid=s.class_id WHERE s.parent_wp_usr_id = %d ORDER BY s.wp_usr_id DESC",$current_user->ID));
                                    //    $wpsp_teacher_data =$wpdb->get_results("SELECT t.first_name,t.last_name, c.teacher_id FROM  wp_wpsp_teacher as t INNER JOIN wp_wpsp_class AS c ON c.teacher_id=t.wp_usr_id where wp_usr_id = '".$wpsp_cls_data[0]->teacher_id."'");
                                        $wpsp_teacher_data = $wpdb->get_results($wpdb->prepare("SELECT t.first_name,t.last_name, c.teacher_id FROM $cteacher AS t INNER JOIN $ctlass AS c ON c.teacher_id=t.wp_usr_id WHERE s.wp_usr_id = %d",$wpsp_cls_data[0]->teacher_id));
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            if($order_date_created != ''){ echo esc_html($newDate = date("F d, Y", strtotime($order_date_created))); }else { echo '-'; }?>
                                        </td>
                                        <td><?php
