						...
						<div class="wpsp-card-body">
							<table id="class_table" class="wpsp-table wpsp-table-bordered wpsp-table-striped" cellspacing="0" width="100%" style="width:100%">
								...
								<tbody>
									...
									//	$wpsp_classes = $wpdb->get_results($wpdb->prepare("SELECT cls.* FROM $ctable cls, $stable st WHERE st.wp_usr_id = %d AND st.class_id=cls.cid",$current_user->ID));
										$wpsp_classes = $wpdb->get_results($wpdb->prepare("SELECT cls.* FROM $ctable AS cls INNER JOIN $stable AS st ON st.class_id = cls.cid WHERE st.wp_usr_id = %d",$current_user->ID));
									} else {
									//	$wpsp_classes =$wpdb->get_results("SELECT DISTINCT cls.* FROM $ctable cls, $stable st where st.parent_wp_usr_id = '".esc_sql($current_user->ID)."' AND st.class_id=cls.cid");
										$wpsp_classes = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT cls.* FROM $ctable AS cls INNER JOIN $stable AS st ON st.class_id = cls.cid WHERE st.parent_wp_usr_id = %d",$current_user->ID));
									}
									$sno=1;
									foreach ($wpsp_classes as $wpsp_class){
										$cid = intval($wpsp_class->cid);
										//$class_students_count = $wpdb->get_var( "SELECT COUNT(`wp_usr_id`) FROM $stable WHERE class_id = '".esc_sql($cid)."'" );
										$class_students_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(`wp_usr_id`) FROM $stable WHERE class_id = %d",$cid));
										$teach_id= intval($wpsp_class->teacher_id);
