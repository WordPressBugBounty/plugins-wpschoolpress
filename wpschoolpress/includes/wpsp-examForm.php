...
if(isset($_GET['id'])){
  ...
  $wpsp_exams = $wpdb->get_results($wpdb->prepare("SELECT * FROM $extable WHERE eid = %d",$examid));
...
$label = isset($_GET['id']) ?  esc_html__( 'Update Exam Information' , 'wpschoolpress' ):  esc_html__('Add Exam Information' , 'wpschoolpress' );
...
<form name="<?php echo esc_attr($formname);?>" action="#" id="<?php echo esc_attr($formname);?>" method="post">
      ...
      <div class="wpsp-card">
        <div class="wpsp-card-head">
          <h3 class="wpsp-card-title">
            <?php echo esc_html($label,'wpschoolpress'); ?>
          </h3>
        </div>
        <div class="wpsp-card-body">
            ...
            <div class="wpsp-col-lg-4 wpsp-col-md-6 wpsp-col-sm-6 wpsp-col-xs-12">
              <div class="wpsp-form-group">
                ...
                <?php
                  ...
                  $classQuery = $wpdb->prepare("SELECT cid,c_name FROM $stable WHERE teacher_id = %d",$cuserId);
								}
                  ...
                  <select name="class_name" data-is_required="<?php echo esc_attr($is_required); ?>" id="class_name" class="wpsp-form-control">
                    ...
                    <?php	foreach($wpsp_classes as $value) {
                      ...
                      <option value="<?php echo esc_attr(intval($value->cid));?>" <?php if($classlistid == $classid) echo esc_html("selected","wpschoolpress"); ?>>
              ...
              <div class="wpsp-form-group exam-subject-list">
                ...
                <div class="exam-class-list">
                  ...
                  if(!empty($classid)){
                    ...
                    $subjectlist = $wpdb->get_results($wpdb->prepare("SELECT sub_name,id FROM $sub_table WHERE class_id = %d",$classid));
                    foreach($subjectlist as $svalue){ ?>
                      <input type="checkbox" name="subjectid[]" value="<?php echo esc_attr($svalue->id); ?>" class="exam-subjects wpsp-checkbox" id="subject-<?php echo esc_attr($svalue->id);?>" <?php if(in_array($svalue->id, $subjectid)){ ?> checked <?php } ?>>
          ...
          <div class="wpsp-row">
            <div class="wpsp-col-xs-12">
              ...
              <a href="<?php echo esc_url(wpsp_admin_url().'sch-exams')?>" class="wpsp-btn wpsp-dark-btn"><?php echo esc_html__( 'Back' , 'wpschoolpress' ); ?>
              </a>
            </div>
          </div>
        </div>
