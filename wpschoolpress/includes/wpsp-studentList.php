      ...
      <table id="student_table" class="wpsp-table" cellspacing="0" width="100%" style="width:100%">
        ...
        <tbody>
          <?php
                ...
                $students = $wpdb->get_results($wpdb->prepare("SELECT * FROM $student_table s, $users_table u where u.ID=s.wp_usr_id AND sid = %d and user_login != %s ORDER BY sid desc",$id,'student'));
                ...
                //  $teacherId = $wpdb->prepare("SELECT teacher_id FROM $class_table WHERE cid IN ($placeholders)",...$subject_ids);
									$courses = get_user_meta( $stinfo->parent_wp_usr_id, '_pay_woocommerce_enrolled_class_access_counter', true );
								  $results = $wpdb->get_results("SELECT s.wp_usr_id,f.student_id,f.order_id FROM $student_table AS s INNER JOIN $tfees AS f ON f.student_id =  s.wp_usr_id");
                  ...
                  <td><?php
							      ...
							      $lname = $stinfo->s_lname;
									  echo esc_html($stinfo->s_fname .' '. $mname .' '.  $lname);?>
                  </td>
                  ...
                  <td align="center">
                    <div class="wpsp-action-col">
                      ...
                      <a href="<?php echo esc_url(wpsp_admin_url().'sch-student&id='. esc_attr($stinfo->wp_usr_id).'&edit=true');?>" title="Edit"><i class="icon dashicons dashicons-edit wpsp-edit-icon"></i></a>
                      <?php if ( in_array( 'administrator', $role )) { ?>
