    	...
    	<div class="wpsp-col-md-12">
      		<div class="wpsp-card">
        		...
        		<div class="wpsp-card-body">
					...
					$check_tt = $wpdb->get_row($wpdb->prepare("SELECT heading FROM $tt_table WHERE class_id = %d and heading!= %s",$class_id,''));
                    $error = ''; $session = array();
					if (!empty($check_tt)) {
						$error = 0;
					...
					//	$chck_hd = $wpdb->get_row("SELECT * from $tt_table where class_id='".esc_sql($class_id)."' and time_id='0' and day='0' and heading!=''");
						$chck_hd = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tt_table WHERE class_id = %d and time_id= %s and day = %s",$class_id,'0','0'));
						if (empty($chck_hd)) {
				...
				//	$clt = $wpdb->get_results("SELECT * FROM $wpsp_subjects_table WHERE class_id='".esc_sql($class_id)."' or class_id=0 order by class_id desc");
					$clt = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpsp_subjects_table WHERE class_id = %d or class_id= %s order by class_id desc",$class_id,'0'));
					if( count($clt) == 0 ) {
						$error = 1;
					...
					if( $error == 0 ) {
						$timetable	=	array();
						$tt_query = $wpdb->prepare("SELECT * FROM $tt_table WHERE class_id = %d and time_id != %s",$class_id,'0');
