		...
		if (!empty($delid)){
			foreach($delid as $uid){
				if ($type == 'student'){
					...
					$parentresult = $wpdb->get_row($wpdb->prepare("SELECT parent_wp_usr_id FROM $student_tbl WHERE wp_usr_id = %d",$uid));
					...
					if($pid!=''){
						$pcount = $wpdb->get_row($wpdb->prepare("SELECT parent_wp_usr_id FROM $student_tbl WHERE parent_wp_usr_id = %d and wp_usr_id != %d",$pid,$uid));
						if(empty($pcount)){
							wp_delete_user(array( 'ID' => $pid ));
						}
					}
					wp_delete_user(array( 'ID' => $uid ));
...
function wpsp_IsNameExist($table, $namecolumn, $name, $idcolumn = null, $id = null, $return = false){
	...
	//	$sql = $wpdb->get_row("select * from $table where UPPER($namecolumn)=UPPER('$name')");
		$sql = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE UPPER(%s) = UPPER(%s)",$namecolumn,$name));
	}else{
	//	$sql = $wpdb->get_row("select $idcolumn from $table where UPPER($namecolumn)=UPPER('$name') and $idcolumn!='$id'");
		$sql = $wpdb->get_row($wpdb->prepare("SELECT idcolumn FROM $table WHERE UPPER(%s) = UPPER(%s) and $idcolumn!=%s",$namecolumn,$name,$id));
	}
	if (!empty($sql)){
...
function isa_test_cron_job_send_mail(){
    ...
    foreach( $mresult  as $value ){
		...
		$studentlists_mapping = $wpdb->get_results($wpdb->prepare("select * from $student_table s, $class_mapping_table m where m.sid = s.sid and m.cid = %d",$value->cid));
            ...
            if($stu->date == "0000-00-00"){
                if($current_date > $end_date){
                  //  $mresult1 = $wpdb->get_row("select * from $class_tbl where cid = ".$stu->cid);
					$mresult1 = $wpdb->get_row($wpdb->prepare("select * from $class_tbl where cid = %d",$stu->cid));
                ...
                //    $wpdb->query("DELETE FROM $class_mapping_table WHERE cid = '".esc_sql($stu->cid)."'");
					$wpdb->query($wpdb->prepare("UPDATE $student_table SET class_id = %d WHERE wp_usr_id = %d",$clswithremove,$stu->wp_usr_id));
					$wpdb->query($wpdb->prepare("DELETE FROM $class_mapping_table WHERE cid = %d",$stu->cid));
...
function wpsp_IsMarkEntered($classid, $subjectid, $examid){
	...
	global $wpdb;
	$mark_tbl = $wpdb->prefix . "wpsp_mark";
	//$mresult = $wpdb->get_results("select COUNT(*) from $mark_tbl where subject_id='".esc_sql($subjectid)."' and class_id='".esc_sql($classid)."' and exam_id='".esc_sql($examid)."'");
	$mresult = $wpdb->get_results($wpdb->prepare("select COUNT(*) from $mark_tbl where subject_id = %d and class_id= %d and exam_id=%d",$subjectid,$classid,$examid));
...
function wpsp_GetMarks($classid, $subjectid, $examid){
	...
	$query = $wpdb->prepare("SELECT * FROM $mtable WHERE subject_id = %d AND class_id = %d AND exam_id = %d ORDER BY mid ASC",$subjectid, $classid, $examid);
...
function wpsp_GetExMarks($subjectid, $examid){
	...
	$ext_marks = $wpdb->get_results($wpdb->prepare("select * from $exmark_tbl where subject_id = %d and exam_id= %d",$subjectid,$examid));
	return $ext_marks;
}
function wpsp_GetRow($table, $id, $select = '*', $column = 'id'){
	wpsp_AllAuthenticate();
	global $wpdb;
//	$row_info = $wpdb->get_row("select $select from $table where $column='".esc_sql($id)."'");
	$row_info = $wpdb->get_row($wpdb->prepare("select $select from $table where $column = %d",$id));
	return $row_info;
}
...
function wpsp_DeleteStudent(){
	...
	$query = $wpdb->prepare("select wp_usr_id,parent_wp_usr_id from $student_tbl where sid = %d",$sid);
	...
	$checkParentExists = $wpdb->get_var($wpdb->prepare("SELECT sid,parent_wp_usr_id FROM $student_tbl WHERE parent_wp_usr_id = %d and wp_usr_id = %d",$parent_id,$std_id));
    if ($checkParentExists > 0) {
		if (get_userdata($result['wp_usr_id'])) {
			wp_delete_user($result['wp_usr_id']);
		}
    }else{
		if (get_userdata($result['wp_usr_id'])) {
        	wp_delete_user($result['wp_usr_id']);
    	}
    	if (get_userdata($result['parent_wp_usr_id'])) {
        	wp_delete_user($result['parent_wp_usr_id']);
    	}
    } 
...
function wpsp_DeleteTeacher(){
	...
	$result = $wpdb->get_var($wpdb->prepare("select wp_usr_id from $teacher_tbl where tid = %d",$tid));
...
function wpsp_UndoImport(){
	...
	$usertable = $wpdb->prefix . "users";
	$query = $wpdb->prepare("select * ,count as totalcount from $importtable where id = %d",$id);
	$result = $wpdb->get_row($query, ARRAY_A);
	...
	if ($type == '1'){
		...
		//	$result1 = $wpdb->get_row("SELECT parent_wp_usr_id FROM $studenttable WHERE wp_usr_id='".esc_sql($value)."'");
			$result1 = $wpdb->get_row($wpdb->prepare("SELECT parent_wp_usr_id FROM $studenttable WHERE wp_usr_id = %d",$value));
			wp_delete_user($result1->parent_wp_usr_id);
...
function wpsp_GetClass(){
	...
	$query = $wpdb->prepare("SELECT * FROM $ctable WHERE cid = %d",$cid);
	$clinfo = $wpdb->get_row($query, ARRAY_A);
...
function wpsp_Updateregisterdeactive(){
	...
	if ($delcl) {
	echo esc_html( 'success' ,'wpschoolpress');
	$query = $wpdb->prepare("SELECT * FROM $temp_tbl WHERE t_id = %d",$cid);
	$userdetails = $wpdb->get_row($query, ARRAY_A);
	...
	echo esc_html( 'Already Dissprove' ,'wpschoolpress');
	}
	wp_die();
}
	...
	$uids = explode(',', sanitize_text_field($_POST['UID']));
 	foreach ($uids as $cid) {
		...
		if($userdetails['t_type'] == "student"){
			...
			echo wp_kses_post($msg);
		}else if($userdetails['t_type'] == "teacher"){
			$userInfo = array(
				'user_login' => $userdetails['t_username'],
				'user_pass' => sanitize_text_field($userdetails['t_password']),
				'user_nicename' => sanitize_text_field($userdetails['t_name']),
...
function wpsp_GetClassName($class_id){
	...
	global $wpdb;
	$class_tbl = $wpdb->prefix . "wpsp_class";
	$query = $wpdb->prepare("SELECT c_name FROM $class_tbl WHERE cid = %d",$class_id);
	$classes = $wpdb->get_row($query, ARRAY_A);
...
function wpsp_GetClassYear(){
	...
	$class_tbl = $wpdb->prefix . "wpsp_class";
	$query = $wpdb->prepare("SELECT c_sdate,c_edate FROM $class_tbl WHERE cid = %d",$class_id);
	$cl_year = $wpdb->get_row($query, ARRAY_A);
...
//	$exinfo = $wpdb->get_row("select count(*) as count from $wpsp_exam_table where e_name='".esc_sql($exam_name)."' and classid = '".esc_sql($class_id)."'");
	$exinfo = $wpdb->get_row($wpdb->prepare("SELECT count(*) as count FROM $wpsp_exam_table WHERE e_name = %s and classid = %d",$exam_name,$class_id));
	$count = $exinfo->count;
...
function wpsp_ExamInfo(){
	...
	$exinfo = $wpdb->get_row($wpdb->prepare("SELECT * FROM $etable WHERE eid = %d ",$eid));
	...
	if (!empty($exinfo)){
		...
		$wpsp_subjects = $wpdb->get_results($wpdb->prepare("SELECT * FROM $stable WHERE class_id = %d ",$exinfo->classid));
...
function wpsp_StudentCount(){
	...
	$stl = $wpdb->get_results($wpdb->prepare("SELECT COUNT(*) as cscount FROM $student_table WHERE class_id = %d ",$classid));
	$count = $stl[0]->cscount;
	if ($count == '0'){
...
function wpsp_getStudentsList(){
	...
	$check_date = $wpdb->get_row($wpdb->prepare("SELECT * FROM $class_table WHERE cid = %d ",$classid));
	...
	$query = $wpdb->prepare("SELECT * FROM $leave_table WHERE leave_date = %d and class_id= %d",$entry_date,$classid);
	$leaveday = $wpdb->get_row($query, ARRAY_A);
	$flag = 0;
	if ($leaveday){
		$msg = sprintf(esc_html('N/A Not Applicable(Date is marked as leave)','wpschoolpress'));
		$flag = 1;
	}

	if ($currentDate < $selected){
		$msg = sprintf(esc_html('Future date is not allowed','wpschoolpress'));
		$flag = 1;
	}	
	...
	$check_attend = $wpdb->get_row($wpdb->prepare("SELECT ab.absents,ab.aid,cls.c_name FROM $att_table ab LEFT JOIN $class_table cls ON cls.cid=ab.class_id WHERE ab.class_id = %d and ab.date = %s",$classid,$entry_date));
                        ...
                        <div class="wpsp-form-group">
                            <table class="wpsp-table wpsp-table-bordered" id="addAttendanceTable" cellspacing="0" width="100%" style="width:100%">
								...
								//	$st = $wpdb->get_row("select wp_usr_id,s_rollno ,CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name from $student_table where sid='$sid' order by s_rollno ASC");
									$st = $wpdb->get_row($wpdb->prepare("select wp_usr_id,s_rollno ,CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name from $student_table WHERE sid = %d order by s_rollno ASC",$sid));
...
function wpsp_AttendanceEntry(){
	...
	$check_attend = $wpdb->get_row($wpdb->prepare("SELECT * FROM $att_table WHERE class_id = %d and date = %s",$class,$entry_date));
	$classname = $wpdb->get_var($wpdb->prepare("SELECT c_name FROM $class_table WHERE cid = %d ",$class));
	...
	if (isset($_POST['Nil']) && $_POST['Nil'] == 'Nil'){
		...
		foreach($abs as $stid){
			...
			if (isset($wpsp_settings_data['absent_sms_alert']) && $wpsp_settings_data['absent_sms_alert'] == 1){
				...
				$studInfo = $wpdb->get_row($wpdb->prepare("SELECT s_phone, s_fname, p_phone FROM $stud_table WHERE wp_usr_id = %d ",$stid));
				$phone = '';
				if (isset($studInfo->p_phone) && !empty($studInfo->p_phone)){
					$phone = $studInfo->p_phone;
				}else{
					$phone = $studInfo->s_phone;
				}
				if(!empty($reason[$stid])){
					$absentreason = 'Your Child ' . $studInfo->s_fname . ' of class ' . esc_html($classname) . ' is absent on ' . esc_html($entry_date) . ' for reason ' . $reason[$stid];
				}else{
					$absentreason = 'Your Child ' . $studInfo->s_fname . ' of class ' . esc_html($classname) . ' is absent on ' . esc_html($entry_date);
				}
				
				
				if($wpsp_settings_data['sch_sms_slaneuser']!= ""){
					$status = apply_filters('wpsp_send_notification_msg', false, $phone, $absentreason);
				} else {
					if($wpsp_settings_data['twilio_api_sid']!= "" && $wpsp_settings_data['twilio_api_auth_token']){
						$status = apply_filters('wpsp_send_notification_msg_twilio', false, $phone, wp_kses_post($absentreason));
					}
					$description = 'studentAttendance';
					$status = apply_filters('wpsp_whatsapp_msg_send', $phone, $description, $studInfo->s_fname, $entry_date);
				}
			}
		}
...
function wpsp_WorkingDays($class){
	...
	$c_dates = $wpdb->get_row($wpdb->prepare("SELECT c_sdate,c_edate FROM $class_table WHERE cid = %d ",$class));
	$today = date('Y-m-d');
...
function wpsp_WorkingDates($start_date, $end_date, $class){
	...
	$query = $wpdb->prepare("SELECT leave_date FROM `$leave_table` where class_id = %d and leave_date BETWEEN %s and %s ",$class,$start_date,$end_date);
	$ldates = $wpdb->get_results($query,ARRAY_A);
...
function wpsp_AttStatus($sdate, $edate, $class){
	...
	$n_attrows = $wpdb->get_row($wpdb->prepare("SELECT count(*) as count FROM `$att_table` where date BETWEEN %s and %s ",$sdate,$edate));
...
function wpsp_AttReport($st_id, $close = 1){
	...
	$stinfo = $wpdb->get_row($wpdb->prepare("SELECT st.class_id, st.s_rollno , CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name, c.c_name, c.c_sdate, c.c_edate from $st_table st LEFT JOIN $class_table c ON c.cid=st.class_id where st.wp_usr_id = %s",$st_id));
	...
	$att_info = $wpdb->get_row($wpdb->prepare("SELECT count(*) as count FROM $att_table WHERE absents like %s ",$ser));
			...
			<table class='wpsp-table'>
				<tbody>
					...
					//	$clasname = $wpdb->get_row("SELECT * FROM $class_table where cid='$id'");
						$clasname = $wpdb->get_row($wpdb->prepare("SELECT * FROM $class_table WHERE cid = %d ",$id));
						//$classname_array[] = $clasname;
					//	$att_info = $wpdb->get_row("select count(*) as count from $att_table WHERE absents LIKE '$ser' AND  class_id='" . esc_sql($id) . "'");
						$att_info = $wpdb->get_row($wpdb->prepare("SELECT count(*) as count FROM $att_table WHERE absents like %s and class_id = %d",$ser,$id));
					//	$attendance_days = $wpdb->get_results("select * from $att_table where class_id='" . esc_sql($id) . "'");
						$attendance_days = $wpdb->get_results($wpdb->prepare("SELECT * FROM $att_table WHERE class_id = %d",$id));
						$present_days = 0;
...
function wpsp_GetAbsentees(){
	...
	$att_info = $wpdb->get_row($wpdb->prepare("SELECT at.*,cl.c_name FROM $att_table at LEFT JOIN $class_table cl ON cl.cid=at.class_id WHERE at.class_id = %d and at.date = %d",$cid,$date));
	...
	if ($absents != 'Nil'){
		...
		//	$st_info = $wpdb->get_row("select CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name from $st_table where wp_usr_id='".esc_sql($st_id)."'");
			$st_info = $wpdb->get_row($wpdb->prepare("SELECT CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name FROM $st_table WHERE wp_usr_id = %d ",$st_id));
...
//	$st_info = $wpdb->get_row("select CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name,cl.c_name from $st_table st LEFT JOIN $class_table cl ON cl.cid=st.class_id where st.wp_usr_id='".esc_sql($st_id)."'");
	$st_info = $wpdb->get_row($wpdb->prepare("SELECT CONCAT_WS(' ', st.s_fname, st.s_mname, st.s_lname ) AS full_name,cl.c_name from $st_table st LEFT JOIN $class_table cl ON cl.cid=st.class_id WHERE st.wp_usr_id = %d ",$st_id));
	$ser = '%' . $st_id . '%';
//	$att_info = $wpdb->get_results("select date, absents from $att_table WHERE absents LIKE '$ser'");
	$att_info = $wpdb->get_results($wpdb->prepare("SELECT date, absents FROM $att_table WHERE absents like %s ",$ser));
	$absents = array();
...
function wpsp_AddSubject(){
	...
	$query = $wpdb->prepare("SELECT sub_name FROM $subject_tbl WHERE class_id = %d",$class_id);
	$c_subjects = $wpdb->get_results($query, ARRAY_A);
		...
		for ($i = 0; $i < $n; $i++){
			if ($subjects[$i] != ''){
				...
				if (array_search($sub_name, $class_subjects) === false){
					$c_subjects = array();
					if (!empty($code[$i])) 
				//	$c_subjects = $wpdb->get_row("select * from $subject_tbl where sub_code=$code[$i] AND class_id='".esc_sql($class_id)."'");
					$c_subjects = $wpdb->get_row($wpdb->prepare("SELECT * FROM $subject_tbl WHERE sub_code = %d AND class_id=",$code[$i],$class_id));
...
function wpsp_SubjectList(){
	...
	$cid = intval($_POST['ClassID']);
	$result['exam'] = $wpdb->get_results($wpdb->prepare("SELECT eid,e_name,subject_id FROM $examtable WHERE classid = %d ",$cid));
	//$wpdb->get_results("select eid,e_name,subject_id from $examtable where classid='".esc_sql($cid)."'");
	if (isset($_POST['get_exam_list']) && intval($_POST['get_exam_list']) == 1){
		if (count($result['exam']) == 1){
			$subjectID = intval($result['exam'][0]->subject_id);
			if (!empty($subjectID)){
				$subject_ids = explode(',', $subjectID); // "1,2,3" -> [1,2,3]
				$subject_ids = array_map('intval', $subject_ids);
				$placeholders = implode(',', array_fill(0, count($subject_ids), '%d'));
				$result['subject'] = $wpdb->get_results($wpdb->prepare("SELECT id,sub_name FROM $sub_table WHERE id IN ($placeholders)",...$subject_ids));
			//	$result['subject'] = $wpdb->get_results("SELECT id,sub_name FROM $sub_table WHERE id IN($subjectID)");
			}
		}
	}else{
		$result['subject'] = $wpdb->get_results($wpdb->prepare("SELECT sub_name,id FROM $sub_table WHERE class_id = %d ",$cid));
...
function wpsp_getMarksubject(){
	...
	$subjectID = $wpdb->get_var($wpdb->prepare("SELECT subject_id FROM $examtable WHERE eid = %d ",$eid));
	if (!empty($subjectID)){
		$subject_ids = explode(',', $subjectID); // "1,2,3" -> [1,2,3]
		$subject_ids = array_map('intval', $subject_ids);
		$placeholders = implode(',', array_fill(0, count($subject_ids), '%d'));
...
function wpsp_SubjectInfo(){
	...
	$cdat = $wpdb->get_row($wpdb->prepare("SELECT * FROM $sub_table WHERE id = %d ",$sid));
...
function wpsp_SaveTimetable(){
	...
	$subcheck_entry = $wpdb->get_row($wpdb->prepare("SELECT sub_teach_id FROM $sub_table WHERE id = %d ",$sid));
	$techerid = isset($subcheck_entry->sub_teach_id) ? sanitize_text_field($subcheck_entry->sub_teach_id) : '';
	$exityesid = 0;
	$exityescname = '';
	if ($techerid > 0 && !empty($techerid)){
		$getsubject = $wpdb->get_results($wpdb->prepare("SELECT id FROM $sub_table WHERE sub_teach_id = %d ",$techerid));
		...
		//	$techbook = $wpdb->get_results("SELECT id,(select c_name from $class_table where cid=class_id) as cname from $time_table where day='".esc_sql($day)."' and subject_id='$subjeid' and time_id='".esc_sql($tid)."'");
			$techbook = $wpdb->get_results($wpdb->prepare("SELECT id,(select c_name from $class_table where cid=class_id) as cname FROM $time_table WHERE day = %s and subject_id= %d and time_id=%d",$day,$subjeid,$tid));
	...
	//	$check_entry = $wpdb->get_row("SELECT * from $time_table where class_id='".esc_sql($cid)."' and day='".esc_sql($day)."' and time_id='".esc_sql($tid)."' and session_id='".esc_sql($sessionid)."' ");
		$check_entry = $wpdb->get_row($wpdb->prepare("SELECT * FROM $time_table WHERE class_id = %d and day= %s and time_id = %d and session_id=%d",$cid,$day,$tid,$sessionid));
		...
		if( isset($_POST['update']) && $_POST['update']=='true' ) {
			...
			foreach(sanitize_price_array($_POST['exmarks']) as $stid=>$field) {
				...
				//	$exmrk_chk	=	$wpdb->get_row("select id from $exmark_table where student_id='".esc_sql($stid)."' and exam_id='".esc_sql($stexam)."' and subject_id='".esc_sql($stsubject)."' and field_id='".esc_sql($flid)."'");
					$exmrk_chk = $wpdb->get_row($wpdb->prepare("SELECT id FROM $exmark_table WHERE student_id = %d and exam_id= %d and subject_id=%d and field_id= %d",$stid,$stexam,$stsubject,$flid));
...
function wpsp_GenSetting(){
	...
	foreach($option_value as $key => $val){
		$check_sett = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpsp_settings_table WHERE option_name = %s ",$key));
		if (empty($check_sett)){
...
function wpsp_ManageGrade(){
	...
	}else if (isset($_POST['actype']) && sanitize_text_field($_POST['actype']) == 'delete'){
...
//	$check_field = $wpdb->get_results("select * from $fields_tbl where field_text='".esc_sql($field)."' and subject_id=".esc_sql($subject_id)."");
	$check_field = $wpdb->get_results($wpdb->prepare("SELECT * FROM $fields_tbl WHERE field_text = %s and subject_id=%d",$key,$subject_id));
...
function wpsp_AddEvent(){
	...
	if (strtotime($sdate) > strtotime($edate)) {
		echo esc_html( 'Event Start date should be older or same then Event end date', 'wpschoolpress' );	
		exit;
	}

	if (strtotime($sdate) === strtotime($edate) && strtotime($stime) > strtotime($etime)) {
		echo esc_html( 'Event Start time should be older then Event end time', 'wpschoolpress' );	
		exit;
	}
...
function wpsp_AddLeaveDay(){
	...
	$avl_dates = $wpdb->get_results($wpdb->prepare("SELECT leave_date FROM $leave_table WHERE class_id = %d ",$class_id));
	$ex_dates = array();
	...
	if ($reason == ''){
		echo  esc_html( 'reason missing', 'wpschoolpress' );
		wp_die();
	}
	if ($sdate == ''){
...
function wpsp_GetLeaveDays(){
	...
	$ldays = $wpdb->get_results($wpdb->prepare("SELECT * FROM $leave_table WHERE class_id = %d and leave_date IS NOT NULL",$cid));
	$sno = 1;
...
function wpsp_UpdateTransport(){
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST'){
		...
		$get_trans = $wpdb->get_row($wpdb->prepare("SELECT * FROM $trans_table WHERE id = %d ",$transid));
		if (!empty($get_trans)){
			$pl = "";
			$item =  apply_filters( 'wpsp_subject_title_item',array());
			if(isset($item['VhName'])){
...
function wpsp_SendMessage(){
	...
	if (isset($_POST['mid']) && !empty($_POST['mid'])){
		...
		$message_block = $get_mb = $wpdb->get_row($wpdb->prepare("SELECT * FROM $messages_table WHERE mid = %d ",$mid));
			...
			if ($msg == ''){
				...
				if ($role != ''){
					...
					if ($explode[0] == 's'){
						...
						$parent_list_1 = $wpdb->get_results($wpdb->prepare("SELECT parent_wp_usr_id,class_id FROM $student_table WHERE parent_wp_usr_id = %d ",0));
					...
					//	$teacher_list = $wpdb->get_results("select DISTINCT t.wp_usr_id from $teacher_table t JOIN $class_table c on t.wp_usr_id = c.teacher_id where c.cid='".esc_sql($class_id)."'");
						$teacher_list = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT t.wp_usr_id FROM $teacher_table t JOIN $class_table c on t.wp_usr_id = c.teacher_id WHERE c.cid = %d ",$class_id));
...
function wpsp_ViewMessage($mid = '', $return = false)
{
	...
	$get_mrow = $wpdb->get_row($wpdb->prepare("SELECT * FROM $msgs_table WHERE mid = %d and (s_id = %d || r_id = %d )",$mid,$cuid,$cuid));
	...
	$content.= '<li><div class="wpsp-row">
            ...
            <div class="wpsp-col-md-11">
						...
						<p>'.nl2br(esc_html($get_mrow->msg)).'</p>
						</div>
        </div>';
...
function view_sub_message($mid){
	...
	$cuid = intval($current_user->ID);
	$msgs_table = $wpdb->prefix . "wpsp_messages";
	$get_mrow = $wpdb->get_results($wpdb->prepare("SELECT * FROM $msgs_table WHERE replay_id = %d ",$mid));
	if(!empty($get_mrow)){
		foreach ($get_mrow as $msg) {
			...
			$content .= '<ul><li><div class="wpsp-row">
				<div class="wpsp-col-md-1" style="float:left">'. get_avatar($msg->r_id, 50).'</div>
					<div class="wpsp-col-md-11">
...
function wpsp_MarkRead($mid, $rid){
	...
	$mrow = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE mid = %d ",$mid));
...
function wpsp_UnreadCount(){
	...
	$mid = $wpdb->get_results($wpdb->prepare("SELECT distinct main_m_id FROM $messages_table WHERE (r_id = %d) and del_stat != %d order by mid DESC ",$uid,$uid));

	foreach($mid as $id){
        $m_id = esc_sql($id->main_m_id);
	//	$sender_id = $wpdb->get_results("select s_id from $messages_table where mid = '$m_id'");
		$sender_id = $wpdb->get_results($wpdb->prepare("SELECT s_id FROM $messages_table WHERE mid = %d ",$m_id));
		...
		if($sender_id[0]->s_id == $uid){
			...
			$unread_msg_count = $wpdb->get_var($wpdb->prepare("SELECT count(*) FROM $messages_table WHERE main_m_id = %d AND s_read = %d AND r_id = %d AND main_m_id not in (select distinct main_m_id from $messages_delete_table where m_id = %d and user_id = %d)",$m_id,0,$uid,$m_id,$uid));
		...
		//	$unread_msg_count = $wpdb->get_var("select count(*) from $messages_table where main_m_id= '$m_id' AND r_read = 0 AND r_id = '$uid' AND main_m_id not in (select  distinct main_m_id from $messages_delete_table where m_id = '$m_id' and user_id = '$uid')");
			$unread_msg_count = $wpdb->get_var($wpdb->prepare("SELECT count(*) FROM $messages_table WHERE main_m_id = %d AND r_read = %d AND r_id = %d AND main_m_id not in (select distinct main_m_id from $messages_delete_table where m_id = %d and user_id = %d)",$m_id,0,$uid,$m_id,$uid));
			$fetch_mess = $fetch_mess + $unread_msg_count;
		}
	}
...
function wpsp_getTeachersList(){
	...
	//	$check_attend = $wpdb->get_results("SELECT * FROM $teacher_attendance_table WHERE leave_date = '".esc_sql($entry_date)."'");
		$check_attend = $wpdb->get_results($wpdb->prepare("SELECT * FROM $teacher_attendance_table WHERE leave_date = %s ",$entry_date));
		$wpdb->prepare("SELECT * FROM $teacher_attendance_table WHERE leave_date = %s ",$entry_date);
...
function wpsp_TeacherAttendanceEntry(){
	...
	$check_attend = $wpdb->get_row($wpdb->prepare("SELECT * FROM $att_table WHERE leave_date = %d ",$entry_date));
	if ($check_attend){
		$del = $wpdb->delete($att_table, array(
...
function wpsp_TeacherAttendanceView(){
	...
	if (isset($_POST['selectedate']) && !empty($_POST['selectedate'])){
		...
		$teacher_attendance_table = $wpdb->prefix . "wpsp_teacher_attendance";
		$teacher_table = $wpdb->prefix . "wpsp_teacher";
		$query = $wpdb->prepare("SELECT * FROM $teacher_attendance_table WHERE leave_date = %s",$entry_date);
		$check_attend = $wpdb->get_results($query, ARRAY_A);
		$allPresent = 0;
		$reasonList = array();
...
function wpsp_addNotify(){
    ...
    if (isset($_POST['type']) && isset($_POST['subject']) && !empty(sanitize_text_field($_POST['subject'])) && isset($_POST['description']) && !empty(sanitize_text_field($_POST['description']))) {
            ...
            if ($receivers == 'alls' || $receivers == 'all') {
				...
				if ($notifyType == 2) { 
					...
					if($role == 'teacher'){
						...
						$results  = $wpdb->get_results($sqlvar, ARRAY_A);
        ...
        if ($notifyType == 1 || $notifyType == 0) { //If notification is mail/All
            ...
            $body = nl2br(esc_html($description . "MyText"));
...
function wpsp_getNotifyInfo(){
	...
	//	$notifyInfo = $wpdb->get_row("Select * from $notify_table where nid= '".esc_sql($notifyID)."'");
		$notifyInfo = $wpdb->get_row($wpdb->prepare("SELECT * FROM $notify_table WHERE nid = %d ",$notifyID));
		...
		if (!empty($notifyInfo)){
			...
			foreach ($notifyRec as $val) {
				if ($val == 'all') {
					...
					$subject_ids = explode(',', $val); // "1,2,3" -> [1,2,3]
					$subject_ids = array_map('intval', $subject_ids);
					$placeholders = implode(',', array_fill(0, count($subject_ids), '%d'));
					//$st = $wpdb->get_results("SELECT CONCAT(s_fname,s_lname) as fname FROM $student_table WHERE wp_usr_id IN($val)");
					$st = $wpdb->get_results($wpdb->prepare("SELECT CONCAT(s_fname,s_lname) as fname FROM $student_table WHERE wp_usr_id IN ($placeholders)",...$subject_ids));
				//	$pt = $wpdb->get_results("SELECT CONCAT(p_fname,p_lname) as fname FROM $student_table WHERE parent_wp_usr_id IN($val)");
					$pt = $wpdb->get_results($wpdb->prepare("SELECT CONCAT(p_fname,p_lname) as fname FROM $student_table WHERE parent_wp_usr_id IN ($placeholders)",...$subject_ids));
				//	$tt = $wpdb->get_results("SELECT CONCAT(first_name,last_name) as fname FROM $teacher_table WHERE wp_usr_id IN($val)");
					$tt = $wpdb->get_results($wpdb->prepare("SELECT CONCAT(first_name,last_name) as fname FROM $teacher_table WHERE wp_usr_id IN ($placeholders)",...$subject_ids));
...
function wpsp_getStudentsAttendanceList(){
	...
	if (!empty($classID)){
		...
		$query = $wpdb->prepare("SELECT * FROM $leave_table WHERE leave_date = %s and class_id = %d",$date,$classID);
		$leaveday = $wpdb->get_row($query, ARRAY_A);
		$attendanceList = '';
		$absentList = array();
		if (!empty($leaveday)){
			...
			$query1 = $wpdb->prepare("SELECT * FROM $att_table WHERE date = %s and class_id = %d",$date,$classID);
			$attendance = $wpdb->get_row($query1, ARRAY_A);
			if (empty($attendance)){
				$msg = __('<span class="wpsp-text-red">N/E</span> No Attendance Entered Yet', 'wpschoolpress');
			}else if (isset($attendance['absents']) && $attendance['absents'] != 'Nil'){
			...
			if (count($studentData) > 0 && empty($msg)){
				...
				foreach($studentData as $student){
					...
					$query3 = $wpdb->prepare("SELECT CONCAT_WS(' ', s_fname, s_mname, s_lname ) AS full_name, wp_usr_id,s_rollno FROM $student_table WHERE sid = %d",$student);
					$studentList = $wpdb->get_results($query3, ARRAY_A);
...
function wpsp_listdashboardschedule(){
	...
	// Event List
	if ($current_user->roles[0] == 'administrator' || $current_user->roles[0] == 'teacher'){
		$query = $wpdb->prepare("SELECT start,end,title FROM $event_table WHERE start >= %d and end <= %d",$start,$end);
		$event_list = $wpdb->get_results($query, ARRAY_A);
	}else{
		$event_list = $wpdb->get_results($wpdb->prepare("SELECT start,end,title FROM $event_table WHERE type = %d and start >= %d and end <= %d",0,$start,$end));
	}
	// Exam List
	$exam_table=$wpdb->prefix."wpsp_exam";
	$class_table=$wpdb->prefix."wpsp_class";
	...
	if ($current_user->roles[0] == 'administrator' || $current_user->roles[0] == 'teacher' ){
		...
		//	$students = $wpdb->get_results("select class_id from $student_table where parent_wp_usr_id='".esc_sql($parent_id)."'");
			$students = $wpdb->get_results($wpdb->prepare("SELECT class_id FROM $student_table WHERE parent_wp_usr_id = %d ",$parent_id));
		}
		...
		if ($current_user->roles[0] == 'student'){
			...
			$students = $wpdb->get_results($wpdb->prepare("SELECT class_id FROM $student_table WHERE wp_usr_id = %d ",$student_id));
		}
		$child_class = array();
		...
		if (!empty($child_cids)){
			$subject_ids = explode(',', $child_cids); // "1,2,3" -> [1,2,3]
			$subject_ids = array_map('intval', $subject_ids);
			$placeholders = implode(',', array_fill(0, count($subject_ids), '%d'));
			if (!empty($child_cids)) {
    			$query = $wpdb->prepare(
        		"SELECT c_name, description, leave_date
         		FROM $leave_table l, $class_table c
         		WHERE l.class_id = c.cid
		...
		//	$leaves = $wpdb->get_results("select c_name, description,leave_date from $leave_table l,$class_table c WHERE l.class_id=c.cid AND c.cid IN($child_cids)", ARRAY_A);
			$leaves = $wpdb->get_results($query, ARRAY_A);
		}
	}
	}
